...
### Step 1: Drive Partitioning & Formatting
...
3. Format the partitions and set up the target mount structure precisely:
   ```bash
...
Wipe any default placeholder configuration generated by the installer environment and clone this exact repository:
```bash
...
### Step 4: Auto-Generate Hardware Configurations
Generate the local hardware mappings and mount options unique to your specific drive layout:
```bash
sudo nixos-generate-config --root /mnt
```
*Note: This will safely generate your individual `hardware-configuration.nix` inside `/mnt/etc/nixos` alongside your existing files without modifying or overwriting your main setup.*
